...
def get_travel_time(origin, destination) -> float:
    ...
    try:
        ...
        return None
@tool
def get_travel_times_node(job_id : str, technician_names: List) -> dict:
...
@tool
def score_rank_node(job_id:str, technician_names:List, travel_times:dict) -> dict:
...
tools = [get_technicians, get_travel_times_node, score_rank_node]
...
You are a smart scheduling assistant. You can call these tools:
- get_technicians: Find technicians available for a job. Returns a dict with 'technician_names'.
- get_travel_times_node: Calculate travel times for technicians to the job. Returns a dict with 'travel_times'.
- score_rank_node: Rank and score technicians. Returns a dict with 'top3_technicians'.

Always use function/tool calls for each step, in this order:
1. get_technicians
2. get_travel_times_node (using job_id and technician_names)
3. score_rank_node (using job_id, technician_names, and travel_times)
Only after all steps, summarize the final top 3 technicians for the user.

Never summarize intermediate results. Only output a summary at the very end, after all tools are called.
"""
    )
...
print("\nFinal NLU Output:")
print(f"JobID: {result.get('job_id')}")
print(f"Constraints: {result.get('constraints')}")
print(f"Priority: {result.get('priority')}")
print(f"Intent: {result.get('intent')}")
print("Messages:", [msg.content for msg in result["messages"] if hasattr(msg, "content")])
print("Technicians:", result.get('technician_names'))
print("Travel Times:", result.get('travel_times'))
print("Top 3:", result.get('top3_technicians'))
